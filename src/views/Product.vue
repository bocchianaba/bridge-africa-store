    ...
    <v-container>
        <v-row justify="center">
            <v-col cols="12" md="6" sm="8" class="text-center">
                ...
                <div>
                    <v-btn 
                        v-if="$store.getters.cart_ids.findIndex(id=>id=product.id)==-1" 
                        class="mr-md-5 mb-sm-5" 
                        outlined 
                        color="orange darken-2" 
                        @click="addToCart(product)"> 
                        <v-icon>mdi-garage-open</v-icon> 
                        Add to Wishlist
                    </v-btn>
                    <v-btn 
                        v-else 
...
export default {
    name: 'Product',
    created(){
        this.$store.dispatch("productAction", this.$route.params.id)
